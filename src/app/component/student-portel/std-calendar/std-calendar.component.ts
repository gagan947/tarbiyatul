...
@Component({
  ...
  styleUrl: './std-calendar.component.css'
})
export class StdCalendarComponent implements OnInit {
  ...
  getStudentEvents() {
    this.apiService.get<{ success: boolean; data: any[] }>('events/student').subscribe({
      next: (response) => {
        console.log('Student Events Data:', response);
